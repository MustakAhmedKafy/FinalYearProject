
<?php
  include 'header.php';
?>


  <!-- banner  -->
  <section class="banner">
    <div class="my-rgba py-5">
      <div class="container">
        <div class="row">
          <div class="col-md-6">
            <p class="h1">Comparison</p>
          </div>
          <div class="col-md-6 d-flex justify-content-end">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item cover-link-color"><a href="index.html">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Comparison</li>
              </ol>
            </nav>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- news -->
  <section class="news my-5">
    <div class="container">
      <div>
        <div class="h1 text-center mt-5">
          Review
        </div>
      </div>
      <div class="row">
        <div class="col-md-12 mt-5">
          <p class="h1 text-center">Comparison</p>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
        <div class="col-md-4 mt-5">
          <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/BUk1o3rTSWg"
              allowfullscreen></iframe>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!--  Pagination  -->
  <section class="pagination py-5 mt-5">
        <div class="container">
            <div class="row">
                <div class="col-md-4 col-sm-12 col-xm-12"></div>
                <div class="col-md-4 col-sm-12 col-xm-12">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <li class="page-item mx-1 "><a class="page-link" href="#">1</a></li>
                            <li class="page-item mx-1"><a class="page-link" href="#">2</a></li>
                            <li class="page-item mx-1"><a class="page-link" href="#">3</a></li>
                            <li class="page-item mx-1 "><a class="page-link" href="#">Next</a></li>
                        </ul>
                    </nav>
                </div>
                <div class="col-md-4 col-sm-12 col-xm-12"></div>
            </div>
        </div>
    </section>

  <!-- footer start-->
<?php
  include 'footer.php';
?>
